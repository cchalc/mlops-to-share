...
```
dbx execute --cluster-name=<name of interactive cluster> --job=dltp8tdylp-sample-integration-test
```

For a test on a automated job cluster, use `launch` instead of `execute`:
```
dbx launch --job=dltp8tdylp-sample-integration-test
```
...
```
dbx launch --job=dltp8tdylp-sample
```
